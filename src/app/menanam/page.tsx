    ...
    try {
      ...
      if (adjustedFertilizerFreq > 0 && values.frequency?.fertilizers?.type) {
        for (
          ...
          current <= end;
          current = new Date(current.getTime() + adjustedFertilizerFreq * 24 * 60 * 60 * 1000)
        ) {
          jadwal.push({
            id_menanam: tanaman_id,
            tanggal: new Date(current.getTime()),
      ...
      const maxDays = Math.max(adjustedWateringFreq, adjustedFertilizerFreq);
      for (let i = 0; ; i++) {
        const day = new Date(start.getTime() + i * 24 * 60 * 60 * 1000);
        if (day > end) break;

        if (i % adjustedWateringFreq === 0) {
          jadwal.push({
            tanggal: day,
            jenis_kegiatan: "Penyiraman",
            status: false,
          ...
          <CardContent className="grid p-0 md:grid-cols-1">
              <Form {...form}>
                        ...
                        <FormControl>
                          <ComboboxDemo
                            ...
                            options={[
                              ...
                              "Irigasi Tetes",
                              "Irigasi Sprinkler",
                              "Irigasi Subsurface (bawah tanah)",
                              "Irigasi Sumur/Pompa",
                              "Irigasi Tadah Hujan"
                            ]}
                            value={field.value || ""}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <ComboboxDemo
                          ...
                          value={field.value?.amount || ""} // pakai optional chaining
                          onChange={(val) => {
                            field.onChange({
                              amount: val, // ganti seluruh object nested
                            });
                          }}
                        />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Pupuk & Frekuensinya</FormLabel>
                      <FormControl>
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Select
                              ...
                              onValueChange={(val) =>
                                field.onChange({ ...(field.value || {}), unit: val })
                              }
                            >
                              <SelectTrigger>
